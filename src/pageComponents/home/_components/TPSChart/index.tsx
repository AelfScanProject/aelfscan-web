import EChartsReactCore from 'echarts-for-react/esm/core.js';
import echarts from 'echarts/lib/echarts';
import 'echarts/lib/chart/line';
import 'echarts/lib/component/legend';
import 'echarts/lib/component/tooltip';
import { memo, useMemo } from 'react';
import './index.css';
import ReactEchartComponent from './useChart';
import clsx from 'clsx';
import { useAppSelector } from '@_store';
import { MULTI_CHAIN } from '@_utils/contant';
import { useSideChain } from '@_hooks/useSelectChain';
interface IDataItem {
  count: number;
  end: number;
  start: number;
}
export interface ITPSData {
  all: IDataItem[];
  owner: IDataItem[];
  mainChain: IDataItem[];
  sideChain: IDataItem[];
}
interface IProps {
  isMobile: boolean;
  data: ITPSData;
}
interface IDataItem {
  count: number;
  end: number;
  start: number;
}

const clsPrefix = 'tps-chart-container';
const symbol =
  'image://data:image/png;base64,';

const createLastDataItem = (data: number) => ({
  value: data,
  symbol,
  symbolSize: [30, 30],
  label: { show: true },
});

const formatXAxisData = (items: IDataItem[]) => {
  return items.map((item) => {
    const startTime = new Date(item.start);
    const hours = startTime.getHours();
    const minutes = startTime.getMinutes().toString().padStart(2, '0');
    return `${hours}:${minutes}`;
  });
};
const getOption = (tpsData, defaultChain, side) => {
  const { owner, all, mainChain, sideChain } = tpsData;
  const multi = defaultChain === MULTI_CHAIN;

  const xAxisData = formatXAxisData(all);
  const allData = all.map((item) => item.count);
  const ownData = multi ? [] : owner.map((item) => item.count);
  const mainData = multi && mainChain ? mainChain.map((item) => item.count) : [];
  const sideData = multi && sideChain ? sideChain.map((item) => item.count) : [];

  const lastIndex = all.length - 1;
  allData[lastIndex] = createLastDataItem(allData[lastIndex] as number);
  if (!multi) {
    ownData[lastIndex] = createLastDataItem(ownData[lastIndex] as number);
  } else {
    mainData[lastIndex] = createLastDataItem(mainData[lastIndex] as number);
    sideData[lastIndex] = createLastDataItem(sideData[lastIndex] as number);
  }

  const seriesData = [
    {
      name: 'All Chains',
      data: allData,
      smooth: true,
      type: 'line',
      itemStyle: { opacity: 0 },
      areaStyle: { color: 'rgba(255, 255, 255, 0.1)' },
    },
  ];

  if (!multi) {
    seriesData.push({
      name: defaultChain,
      data: ownData,
      smooth: true,
      type: 'line',
      itemStyle: { opacity: 0 },
      areaStyle: { color: 'rgba(255, 255, 255, 0.1)' },
    });
  } else {
    seriesData.push(
      {
        name: 'AELF',
        data: mainData,
        smooth: true,
        type: 'line',
        itemStyle: { opacity: 0 },
        areaStyle: { color: 'rgba(255, 255, 255, 0.1)' },
      },
      {
        name: side,
        data: sideData,
        smooth: true,
        type: 'line',
        itemStyle: { opacity: 0 },
        areaStyle: { color: 'rgba(255, 255, 255, 0.1)' },
      },
    );
  }

  return {
    color: ['#5c28a9', '#266CD3', '#EEB420'],
    legend: {
      show: true,
      data: !multi ? ['All Chains', defaultChain] : ['All Chains', 'AELF', side],
      top: 'top',
      left: 'right',
    },
    grid: {
      left: '0%',
      right: '16px',
      bottom: '16px',
      top: '16px',
      containLabel: true,
    },
    tooltip: {
      show: true,
      trigger: 'axis',
      axisPointer: {
        type: 'line',
        label: { backgroundColor: '#6a7985' },
      },
    },
    xAxis: {
      type: 'category',
      boundaryGap: false,
      data: xAxisData,
      axisLine: { show: false, lineStyle: { color: '#999' } },
      splitLine: { show: true },
    },
    yAxis: {
      type: 'value',
      axisLine: { show: false, lineStyle: { color: '#999' } },
      splitLine: { show: false },
      axisTick: { show: true },
    },
    series: seriesData,
  };
};
function TPSChart({ isMobile, data }: IProps) {
  const { defaultChain } = useAppSelector((state) => state.getChainId);
  const sideChain = useSideChain();
  const option = useMemo(() => {
    return getOption(data, defaultChain, sideChain);
  }, [data, defaultChain, sideChain]);
  return (
    <div className="w-full">
      <div className={clsx(clsPrefix, isMobile && `${clsPrefix}-mobile`)}>
        <h3>Transactions Per Minute</h3>
        <ReactEchartComponent className="chart" option={option}></ReactEchartComponent>
      </div>
    </div>
  );
}

export default memo(TPSChart);
