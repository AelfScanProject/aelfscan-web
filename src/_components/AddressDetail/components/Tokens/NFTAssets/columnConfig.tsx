...
export default function getColumns(chain, sortedInfo): ColumnsType<NftsItemType> {
  return [
    {
      dataIndex: 'token',
      width: 235,
    ...
    },
    {
      ...
      render: (collection) =>
        !collection && (
          <Link href={`/${chain}/token/${collection.symbol}`}>
    ...
    },
    {
      ...
      render: (text) => <span className="mx-1 inline-block text-base-100">{formatDate(text, 'Date Time (UTC)')}</span>,
    },
  ];
}
